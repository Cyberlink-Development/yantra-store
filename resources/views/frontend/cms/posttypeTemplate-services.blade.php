          ...
          <div class="bg-white rounded-lg">
            <div class="p-2 h-320">
              <a href="{{ route('page.pagedetail', $row->uri) }}">
                <img
                  src="{{ $row->thumbnail ? asset('uploads/thumbnails/' . $row->thumbnail) : asset('theme-assets/img/service/cloud.png')}}"
                  class="service-list-img" alt="{{$row->post_title}}">
              </a>
            ...
            <div class="container">
              <div class="row">
                ...
                @else
                  <div class="col-12 p-0">
                    <a href="#quote" data-toggle="modal" class="quote-badge text-center" data-service-id="{{ $row->id }}" data-price="{{ $row->price }}">Get A Quote</a>
                  </div>
                @endif
              </div>
            </div>
            ...
            <div class="row">
              ...
              <div class="col-md-12">
                <div class="form-group">
                  ...
                  <input class="custom-select" name="service_name" id="quote-service" readonly required>
                </div>
              </div>
              <div class="col-md-12">
                <div class="form-group">
...
@push('scripts')
  <script>
    $('#quote').on('show.bs.modal', function (event) {
      ...
      // Get the service title from the clicked element
      var serviceTitle = button.closest('.bg-white').find('.services-span').text();
      
      // Update modal header based on whether price exists
      if (price) {
      ...
      // Set the service name in the input field
      modal.find('#quote-service').val(serviceTitle);
      
      // Set the service ID in hidden field
      modal.find('#quote-service-id').val(serviceId);
