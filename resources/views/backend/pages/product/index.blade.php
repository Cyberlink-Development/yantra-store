...
@section('breadcrum')
    @include('backend.layouts.breadcrum', ['title' => 'Product List', 'actionLabel' => 'Create', 'actionLink' => route('product.create'), 'backLabel' => '', 'backLink' => ''])
@endsection
@section('content')
<div class="card">
            ...
            <tbody>
                @foreach($products as $key => $product)
                    <tr>
                        ...
                        <td>
                            <input type="checkbox" class="toggle-status" data-id="{{ $product->id }}" name="status"
                                onclick="updateStatus(this, {{$product->id}},'{{getModelPathFromData($product)}}')" {{ $product->status == '1' ? 'checked' : '' }}>
                            <!-- @if($product->status==1)
                        ...
                        <td>
                            <img src="{{$img->get_main_image($product->id) ? asset('/images/products/' . $img->get_main_image($product->id)) : asset('images/default-thumbnail.jpeg')}}"
                                width="100px" alt="product">
                        </td>
                        <td>
                            {{ $product->stock }}
                        ...
                        <td>
                            ...
                            <a class="btn btn-sm btn-outline-primary confirm"
                                ...
                                onclick="return confirm('Confirm Delete?')"><i class="fa fa fa-trash"></i></a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
