...
@section('breadcrum')
    @include('backend.layouts.breadcrum', ['title' => 'All Posts','actionLabel'=>'Create','actionLink'=> route('admin.post.create',Request::segment(3)) ])
@endsection
@section('content') 
                    ...
                    <table id="package_table" class="table table-bordered datatable123">
                        ...
                        <tbody>
                            @foreach($data as $key=>$value)
                                <tr>
                                    <td>{{$key+=1}}</td>
                                    <td><a>{{ ucfirst($value->post_title) }}</a></td>
                                    <td>
                                    ...
                                    <td>
                                        <a class="btn btn-outline-primary btn btn-sm confirm"  href="{{route('admin.post.edit',[$posttype->uri, $value->id])}}"><i class="fa fa fa-edit"></i> </a>
                                        <form action="{{ route('admin.post.destroy', [$posttype->uri, $value->id]) }}" method="POST" style="display:inline;">
                                            @csrf
                                            @method('DELETE')
...
@push('scripts')
    ...
    <script>
        $(document).on('click', '.status-btn', function() {
            ...
            $.ajax({
                url: '{{ url("admin/post") }}/' + id + '/toggle-status',
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
